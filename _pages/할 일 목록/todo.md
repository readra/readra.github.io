...
10. 면접 질문 스터디
- with / resursive with 문을 사용하지 말라고 권고가 내려왔다 이유가 무엇일거라고 생각하며 그에 대한 해결 방안
- 화면단에서 parameter 변조로 권한이 없는 화면이 보이는 현상이 있다고 가정할 때, 추후에 이런 점을 어떤 방식으로 보완해서 설계를 할 것인지
- 거래 내역 조회를 하는 쿼리가 해시 조인으로 되어 있는데, 해시 조인이 PGT? 로 인해 CPU가 높아서 DBA가 쿼리 튜닝을 요청했다. 어떻게 조치할 것인가